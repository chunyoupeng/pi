...
export abstract class TuiBase extends Container implements TUI {
	...
	private handleTerminalInput(data: string): void {
		...
		// Track focus state here but keep the event flowing so input listeners
		// (e.g. the alt screen's selection handling) can also react to it.
		...
		// Focus reporting events were handled above; don't let them reach keyboard parsing.
		if (data === "\x1b[I" || data === "\x1b[O") {
			return;
		}
